  ...
  reminderTimeoutId = setTimeout(() => {
    try {
      new Notification("Roz ki Dua", {
        body: "Aaj ki duayein padhi? Ek minute nikaalein aur padh lein 🤲",
        icon: "icon-192.png",
      });
    } catch (e) {}
...
function renderNamazView() {
  ...
  const rows = [
    { name: "Fajr", time: to12Hour(t.Fajr), rakat: "2 Sunnat + 2 Farz" },
    { name: "Sunrise", time: to12Hour(t.Sunrise), rakat: "(Namaz nahi — sirf waqt ka nishaan)" },
    { name: "Zuhr", time: to12Hour(t.Dhuhr), rakat: "4 Sunnat + 4 Farz + 2 Sunnat + 2 Nafl" },
    { name: "Asr", time: to12Hour(t.Asr), rakat: "4 Sunnat + 4 Farz" },
    { name: "Maghrib", time: to12Hour(t.Maghrib), rakat: "3 Farz + 2 Sunnat + 2 Nafl" },
    { name: "Isha", time: to12Hour(t.Isha), rakat: "4 Sunnat + 4 Farz + 2 Sunnat + 2 Nafl + 3 Wajib + 2 Nafl" },
    { name: "Tahajjud", time: calcTahajjudWindow(t.Maghrib, t.Fajr) || "—", rakat: "8 Rakat Nafl (2-2 karke) — Witr agar Isha ke baad na padha ho to yahin ada karein" },
  ];

  return `
    <div class="namaz-wrap">
      <button class="namaz-enable-btn" style="margin-bottom:16px;" onclick="openQiblaModal()">🧭 Qibla Direction Dekhein</button>
...
function renderMainContent() {
  ...
  `;
  container.innerHTML = getSpecialBannersHTML() + amalCard + todaysSet.map(duaCardHTML).join("");
}
