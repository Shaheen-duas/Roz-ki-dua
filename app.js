...
async function openSurah(number) {
  ...
  try {
    const res = await fetch(`https://api.alquran.cloud/v1/surah/${number}/editions/quran-simple,en.sahih,hi.hindi`);
    const json = await res.json();
    const arabicAyahs = json.data[0].ayahs;
    const enAyahs = json.data[1].ayahs;
    const hiAyahs = json.data[2] ? json.data[2].ayahs : [];
    surahCache[number] = arabicAyahs.map((a, i) => ({
      numberInSurah: a.numberInSurah,
      ar: a.text,
      en: enAyahs[i] ? enAyahs[i].text : "",
      hi: hiAyahs[i] ? hiAyahs[i].text : "",
    ...
    if (ayahs === undefined) {
      ...
      html += ayahs
        ...
        <div class="ayah-card">
          ...
          ${state.langPref === "hi" && a.hi ? `<div class="ayah-en" style="font-style:normal;">${a.hi}</div>` : ""}
        </div>`;
        })
        .join("");
    }
